...
const verifyIncludeRawText = (bibfile) => {
  ...
  it(`include raw_text: ${name}`, () => {
    ...
    clean(found)
    expect(found.entries['1'].raw_text).to.match(/^@article\{Sen\.2016\.BV,\n abstract = \{We construct the quantum.+year = \{2016\}\n\}$/s)
    expect(found.entries['2'].raw_text).to.match(/^@article\{Smolin\.2013\.Classical,\n.+year = \{2013\}\n\}$/s)
    expect(found.entries['3'].raw_text).to.match(/^@article\{Ronnow\.2014\.Defining,\n.+year = \{2014\}\n\}$/s)
  })
}
...
verifyIncludeLocation(path.join(fixtures, 'arXiv identifiers in BibLaTeX export #460.bibtex.bib'))
verifyIncludeRawText(path.join(fixtures, 'arXiv identifiers in BibLaTeX export #460.bibtex.bib'))
